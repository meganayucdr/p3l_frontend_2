...
<script>
  ...
  export default {
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Tambah Fasilitas' : 'Ubah Fasilitas'
      }
    },
    ...
    methods: {
      fetchItem() {
        const URL = '/api/facility?name=' + this.key
        axios.get(URL)
          .then(response => {
      ...
      deleteItem (item) {
        const URL = '/api/facility/' + item.id
      ...
      save () {
        if (this.$refs.form.validate()) {
          if(this.editedItem.id == "") {
            ...
            const URL = '/api/facility/' + this.editedItem.id
